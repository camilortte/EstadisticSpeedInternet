...
def save_in_file(download,upload,server):
    download=str(download)
    upload=str(upload)    
    latency=str(server.get('latency'))
    host=str(server.get('host'))
    sponsor=str(server.get('sponsor'))    
    date=str(datetime.datetime.now().strftime("%d-%m-%Y %H:%M"))
    f = open('salida.txt','a')
    f.write(download+","+upload+","+latency+","+host+","+sponsor+","+date+"\n")
    f.close() 
...
def speedtest():
    ...
    #MI modificacion
    var_download=((dlspeed / 1000 / 1000) * args.units[1])    
    var_upload=((ulspeed / 1000 / 1000) * args.units[1])   
    save_in_file(var_download,var_upload,best)
    ##*********************************************************
